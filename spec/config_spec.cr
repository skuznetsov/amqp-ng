...
describe Amqp::Config do
  ...
  it "parses supported query keys" do
    cfg = Amqp::Config.parse(
      "amqp://user:p%40ss@example.com/app?heartbeat=7&channel_max=9&frame_max=4096&connect_timeout=3&tcp_nodelay=true&buffer_size=32768&recovery=full&product=p&information=i"
    )
  ...
  it "lets keyword arguments override URI query values" do
    cfg = Amqp::Config.parse(
      "amqp://u:p@example.com/q?heartbeat=7&channel_max=9&frame_max=4096&connect_timeout=3&tcp_nodelay=true&buffer_size=32768&recovery=full&product=query&information=query",
      user: "kw-user",
      password: "kw-pass",
      vhost: "kw-vhost",
      heartbeat: 8.seconds,
