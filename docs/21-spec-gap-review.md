...
- Latest no-broker full `crystal spec --error-trace` passes: 228 examples, 0 failures, 0 errors, 92 pending.
...
- `tools/perf_publish.cr` now exists as a local publish benchmark witness, and the manual `Perf Smoke` workflow runs it against RabbitMQ/LavinMQ with tiny counts. `tools/perf_smoke_assert.cr` validates required positive JSON lanes. Normative thresholded `spec/perf/` remains a roadmap item.
- The manual `Release Gates` workflow provisions RabbitMQ/LavinMQ TLS and runs the opt-in backpressure and Docker chaos specs against RabbitMQ/LavinMQ.
...
- Docs/config-surface lint now keeps URI query keys in docs and README aligned with `Config::RECOGNIZED_QUERY_KEYS`.
...
- RabbitMQ/LavinMQ live TLS success is covered by the manual
  `Release Gates` workflow. Bad-CA and expired-certificate rows remain
