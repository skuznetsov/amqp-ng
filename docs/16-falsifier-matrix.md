...
`T-PERF-STATS-001` have default-off carriers under `spec/perf/`. The
remaining `T-PERF-*` rows are reserved roadmap falsifiers until their
carriers exist. None of these rows are current release-blocking checks.
