...
- [x] Bound inbound content body size with configurable `max_body_size`.
  ...
  - Evidence: full no-broker `/opt/homebrew/bin/crystal spec --error-trace` exits 0: 228 examples, 0 failures, 0 errors, 92 pending.
  - Cutline: in-flight and mailbox capacity budgets are tracked below; process-wide memory budgets remain future work.
- [x] Bound per-connection in-flight content body assembly bytes.
  ...
  - Cutline: mailbox capacity guard is tracked below; actual queued-byte accounting and per-process memory budgets remain future work.
- [x] Bound worst-case subscription mailbox body bytes.
  - Problem: even after in-flight assembly is bounded, a large subscription `buffer` can retain many delivered message bodies.
  - Progress: added `Config#max_subscription_mailbox_bytes`, URI/keyword parsing with positive `UInt64` validation, and a subscription registration guard that rejects `buffer * max_body_size` above the cap. The default is `1024 * max_body_size` to preserve the existing low-level consume buffer default.
  - Evidence: `/opt/homebrew/bin/crystal spec spec/config_spec.cr spec/channel_spec.cr --error-trace` exits 0: 49 examples, 0 failures, 0 errors, 27 pending.
  - Evidence: `/opt/homebrew/bin/crystal spec spec/docs_falsifier_link_spec.cr --error-trace` exits 0: 4 examples, 0 failures, 0 errors, 0 pending.
  - Evidence: full no-broker `/opt/homebrew/bin/crystal spec --error-trace` exits 0: 237 examples, 0 failures, 0 errors, 92 pending.
  - Cutline: this is a worst-case capacity guard, not actual queued-byte accounting or a process-wide memory budget.
...
- [x] Add route-fanout and body-size publish attribution.
  - Frame: `Window` = body-size and route-fanout uncertainty after the single-publish probe; `Transport` = configured route/body sweep -> live publish lanes plus synthetic frame-encode stages; `Potential` = `(body_size_slope_unknown, route_fanout_slope_unknown, local_encode_vs_broker_ingest_uncertainty, benchmark_noise)`.
  - Progress: added `AMQP_BENCH_ROUTE_COUNTS` round-robin route lanes, `AMQP_BENCH_BODY_SIZES` live single-publish body-size lanes, and matching `encode_empty_publish_frames_body_<n>` synthetic stages.
  - Evidence: LavinMQ 2.4.0 release probe `.tmp/bench/current_ng_lavinmq_route_body_stage_20260520.json` passes `tools/perf_smoke_assert.cr` with 34 metrics and 12 stages. It reports live body-size medians body 0 ~770.2k, 64 ~575.6k, 256 ~331.1k, 1024 ~242.5k msg/s; synthetic encode medians body 0 ~11.9M, 64 ~9.9M, 256 ~6.1M, 1024 ~2.48M ops/s; route fanout medians 2 routes ~511.0k, 4 ~394.0k, 8 ~312.1k msg/s.
  - Insight: body-size cost is visible both in local frame construction and live broker path; route fanout has a descending signal at 2/4/8 routes. The next optimization should favor batching/prepared fixed-route guidance or a lower-level socket/body-write corridor only if a writev-like transport becomes available; there is not yet evidence for a safe semantic-preserving runtime rewrite.
- [x] Add run-local perf recommendation tooling.
  - Problem: benchmark JSON now has enough attribution lanes to guide caller behavior, but `perf_smoke_assert` and `perf_threshold_assert` only validate shape/thresholds.
  - Progress: added `tools/perf_recommend.cr` and `tools/perf_recommendations.cr`. The tool compares lanes within one benchmark artifact and prints conservative recommendations for batch publish, prepared fixed-route publishers, route fanout, and body-size dominated workloads.
...
- [x] Add default-off `spec/perf` carrier for `T-PERF-CONS-001`.
  - Frame: `Window` = roadmap manual-ack consume row without an executable carrier; `Transport` = preloaded queue -> subscription receive/ack loop -> local result artifact; `Potential` = `(reserved_perf_rows, unmeasured_consume_ack_latency, mailbox_capacity_uncertainty, harness_bootstrap_work)`.
  - Progress: added `spec/perf/t_perf_cons_001_spec.cr` plus a shared `PerfSpecHelper.legal_subscription_buffer`. The spec is gated by `AMQP_PERF_LIVE=1`, preloads warm-up and measured queues outside timed receive/ack sections, checks `AMQP_PERF_CONS_001_MIN`, and records message count, prefetch, batch size, body size, and consume buffer in result metadata.
  - Evidence: `/opt/homebrew/bin/crystal spec spec/perf/t_perf_pub_001_spec.cr spec/perf/t_perf_pub_002_spec.cr spec/perf/t_perf_multichan_001_spec.cr spec/perf/t_perf_cons_001_spec.cr spec/docs_falsifier_link_spec.cr --error-trace` exits 0: 8 examples, 0 failures, 0 errors, 4 pending.
  - Evidence: `AMQP_PERF_LIVE=1 AMQP_PERF_CONS_MESSAGES=100 AMQP_PERF_CONS_WARMUP_MESSAGES=10 AMQP_PERF_CONS_001_MIN=1 /opt/homebrew/bin/crystal spec spec/perf/t_perf_cons_001_spec.cr --error-trace` exits 0 and remains pending when no local broker is reachable.
  - Evidence: `/opt/homebrew/bin/crystal build spec/perf/t_perf_cons_001_spec.cr --release --no-codegen --error-trace`, `/opt/homebrew/bin/crystal tool format --check src spec tools/perf_publish.cr tools/capture_proxy.cr tools/perf_smoke_assert.cr tools/perf_thresholds.cr tools/perf_threshold_assert.cr tools/perf_recommendations.cr tools/perf_recommend.cr tools/perf_compare_report.cr tools/perf_compare.cr`, and full `/opt/homebrew/bin/crystal spec --error-trace` exit 0; full suite reports 257 examples, 0 failures, 0 errors, 96 pending.
  - Cutline: this measures manual-ack consume carrier shape only when a live broker is supplied; actual queued-byte accounting and normalized broker/compiler baselines remain future work.
- [x] Add default-off `spec/perf` carrier for `T-PERF-HANDSHAKE-001`.
  - Frame: `Window` = roadmap plain/TLS handshake latency row without an executable carrier; `Transport` = sequential `Amqp.connect` samples -> p99 calculation -> local result artifact; `Potential` = `(reserved_perf_rows, unverifiable_handshake_latency_claims, credential_leak_surface, harness_bootstrap_work)`.
  - Progress: added `spec/perf/t_perf_handshake_001_spec.cr` plus shared percentile, connect-timing, TLS-context, reachability, and redacted-URL helpers. Existing perf carriers now record redacted broker URLs in result metadata.
  - Evidence: `/opt/homebrew/bin/crystal spec spec/perf/t_perf_handshake_001_spec.cr spec/perf/t_perf_pub_001_spec.cr spec/perf/t_perf_pub_002_spec.cr spec/perf/t_perf_multichan_001_spec.cr spec/perf/t_perf_cons_001_spec.cr spec/docs_falsifier_link_spec.cr --error-trace` exits 0: 10 examples, 0 failures, 0 errors, 6 pending.
  - Evidence: `AMQP_PERF_LIVE=1 AMQP_PERF_HANDSHAKE_CONNECTIONS=2 AMQP_PERF_HANDSHAKE_P99_MS=1 /opt/homebrew/bin/crystal spec spec/perf/t_perf_handshake_001_spec.cr --error-trace` exits 0 and remains pending when no local plain/TLS broker is reachable.
  - Evidence: `/opt/homebrew/bin/crystal build spec/perf/t_perf_handshake_001_spec.cr --release --no-codegen --error-trace`, `/opt/homebrew/bin/crystal tool format --check src spec tools/perf_publish.cr tools/capture_proxy.cr tools/perf_smoke_assert.cr tools/perf_thresholds.cr tools/perf_threshold_assert.cr tools/perf_recommendations.cr tools/perf_recommend.cr tools/perf_compare_report.cr tools/perf_compare.cr`, and full `/opt/homebrew/bin/crystal spec --error-trace` exit 0; full suite reports 259 examples, 0 failures, 0 errors, 98 pending.
  - Cutline: this verifies carrier shape and result hygiene by default; actual p99 latency remains live-broker and host-specific until a broker run or perf CI supplies baselines.
- [x] Add default-off `spec/perf` carrier for `T-PERF-STATS-001`.
  - Frame: `Window` = roadmap stats read overhead row without an executable carrier; `Transport` = `Amqp::Stats#snapshot` loop -> average microseconds per call -> local result artifact; `Potential` = `(reserved_perf_rows, unmeasured_observability_cost, harness_bootstrap_work)`.
  - Progress: added `spec/perf/t_perf_stats_001_spec.cr`. The spec is gated by `AMQP_PERF_LIVE=1`, requires no broker, checks `AMQP_PERF_STATS_001_MAX_US`, and records iteration count, elapsed time, counter count, and sink metadata.
  - Evidence: `/opt/homebrew/bin/crystal spec spec/perf/t_perf_stats_001_spec.cr spec/perf/t_perf_handshake_001_spec.cr spec/perf/t_perf_pub_001_spec.cr spec/perf/t_perf_pub_002_spec.cr spec/perf/t_perf_multichan_001_spec.cr spec/perf/t_perf_cons_001_spec.cr spec/docs_falsifier_link_spec.cr --error-trace` exits 0: 11 examples, 0 failures, 0 errors, 7 pending.
  - Evidence: `AMQP_PERF_LIVE=1 AMQP_PERF_STATS_READS=1000 AMQP_PERF_STATS_001_MAX_US=100 /opt/homebrew/bin/crystal spec spec/perf/t_perf_stats_001_spec.cr --error-trace` exits 0: 1 example, 0 failures, 0 errors, 0 pending, and writes ignored local result artifacts.
  - Evidence: `/opt/homebrew/bin/crystal build spec/perf/t_perf_stats_001_spec.cr --release --no-codegen --error-trace`, `/opt/homebrew/bin/crystal tool format --check src spec tools/perf_publish.cr tools/capture_proxy.cr tools/perf_smoke_assert.cr tools/perf_thresholds.cr tools/perf_threshold_assert.cr tools/perf_recommendations.cr tools/perf_recommend.cr tools/perf_compare_report.cr tools/perf_compare.cr`, and full `/opt/homebrew/bin/crystal spec --error-trace` exit 0; full suite reports 260 examples, 0 failures, 0 errors, 99 pending.
  - Cutline: this measures local snapshot read overhead only; scrape cadence, exporter overhead, and CI-normalized host baselines remain future work.
- [x] Add default-off `spec/perf` carrier for `T-PERF-MEM-001..002`.
  - Frame: `Window` = roadmap idle-connection and buffered-delivery memory rows without executable carriers; `Transport` = retained live AMQP object graph -> `GC.stats.heap_size` delta -> local result artifact; `Potential` = `(reserved_perf_rows, unmeasured_memory_bounds, broker_reachability_uncertainty, harness_bootstrap_work)`.
  - Progress: added `spec/perf/t_perf_mem_001_002_spec.cr`. The spec is gated by `AMQP_PERF_LIVE=1`, measures idle connection heap bytes per connection and buffered unread no-ack delivery heap bytes per delivery, checks caller-provided byte bounds, and records heap before/after/delta metadata.
  - Evidence: `/opt/homebrew/bin/crystal spec spec/perf/t_perf_mem_001_002_spec.cr spec/perf/t_perf_stats_001_spec.cr spec/perf/t_perf_handshake_001_spec.cr spec/perf/t_perf_pub_001_spec.cr spec/perf/t_perf_pub_002_spec.cr spec/perf/t_perf_multichan_001_spec.cr spec/perf/t_perf_cons_001_spec.cr spec/docs_falsifier_link_spec.cr --error-trace` exits 0: 13 examples, 0 failures, 0 errors, 9 pending.
  - Evidence: `AMQP_PERF_LIVE=1 AMQP_PERF_MEM_CONNECTIONS=1 AMQP_PERF_MEM_DELIVERIES=1 AMQP_PERF_MEM_001_MAX_BYTES=999999999 AMQP_PERF_MEM_002_MAX_BYTES=999999999 /opt/homebrew/bin/crystal spec spec/perf/t_perf_mem_001_002_spec.cr --error-trace` exits 0 and remains pending when no local broker is reachable.
  - Evidence: `/opt/homebrew/bin/crystal build spec/perf/t_perf_mem_001_002_spec.cr --release --no-codegen --error-trace`, `/opt/homebrew/bin/crystal tool format --check src spec tools/perf_publish.cr tools/capture_proxy.cr tools/perf_smoke_assert.cr tools/perf_thresholds.cr tools/perf_threshold_assert.cr tools/perf_recommendations.cr tools/perf_recommend.cr tools/perf_compare_report.cr tools/perf_compare.cr`, and full `/opt/homebrew/bin/crystal spec --error-trace` exit 0; full suite reports 262 examples, 0 failures, 0 errors, 101 pending.
  - Cutline: this measures coarse `GC.stats.heap_size` deltas only when a live broker is supplied; retained heap granularity, allocator segment reuse, and CI-normalized host baselines remain future work.
- [x] Add default-off `spec/perf` carrier for `T-PERF-GC-001`.
  - Frame: `Window` = roadmap GC pressure row without an executable carrier and with stale `GC.stats.collections` wording for current Crystal; `Transport` = live fire-and-forget publish loop -> `GC.stats.total_bytes` delta -> bytes/message artifact; `Potential` = `(reserved_perf_rows, stale_gc_metric_claims, unmeasured_allocation_pressure, broker_reachability_uncertainty)`.
  - Progress: added `spec/perf/t_perf_gc_001_spec.cr` and updated `docs/14-performance-contract.md` to use the current Crystal `GC.stats.total_bytes` allocation-pressure signal. The spec is gated by `AMQP_PERF_LIVE=1`, checks `AMQP_PERF_GC_001_MAX_BYTES_PER_MESSAGE`, and records total byte counters plus body size and message count.
  - Evidence: `/opt/homebrew/bin/crystal spec spec/perf/t_perf_gc_001_spec.cr spec/perf/t_perf_mem_001_002_spec.cr spec/perf/t_perf_stats_001_spec.cr spec/perf/t_perf_handshake_001_spec.cr spec/perf/t_perf_pub_001_spec.cr spec/perf/t_perf_pub_002_spec.cr spec/perf/t_perf_multichan_001_spec.cr spec/perf/t_perf_cons_001_spec.cr spec/docs_falsifier_link_spec.cr --error-trace` exits 0: 14 examples, 0 failures, 0 errors, 10 pending.
  - Evidence: `AMQP_PERF_LIVE=1 AMQP_PERF_GC_MESSAGES=1 AMQP_PERF_GC_001_MAX_BYTES_PER_MESSAGE=999999999 /opt/homebrew/bin/crystal spec spec/perf/t_perf_gc_001_spec.cr --error-trace` exits 0 and remains pending when no local broker is reachable.
  - Evidence: `/opt/homebrew/bin/crystal build spec/perf/t_perf_gc_001_spec.cr --release --no-codegen --error-trace`, `/opt/homebrew/bin/crystal tool format --check src spec tools/perf_publish.cr tools/capture_proxy.cr tools/perf_smoke_assert.cr tools/perf_thresholds.cr tools/perf_threshold_assert.cr tools/perf_recommendations.cr tools/perf_recommend.cr tools/perf_compare_report.cr tools/perf_compare.cr`, and full `/opt/homebrew/bin/crystal spec --error-trace` exit 0; full suite reports 263 examples, 0 failures, 0 errors, 102 pending.
  - Cutline: this measures allocation pressure, not GC pause time or collection count; live broker throughput, allocator segment reuse, and CI-normalized host baselines remain future work.
- [x] Add default-off `spec/perf` carrier for `T-PERF-RECOV-001`.
  - Frame: `Window` = final roadmap recovery dead-window row without an executable carrier; `Transport` = Docker broker restart -> recovery state -> first successful `publish_confirm` -> median artifact; `Potential` = `(reserved_perf_rows, unmeasured_recovery_dead_window, broker_restart_uncertainty, harness_bootstrap_work)`.
  - Progress: added `spec/perf/t_perf_recov_001_spec.cr`. The spec is gated by `AMQP_PERF_LIVE=1`, `AMQP_CHAOS_DOCKER_CONTAINER`, Docker availability, and broker reachability; it checks `AMQP_PERF_RECOV_001_MAX_MS` and records sample count, min/max, recovery delay, timeout, and redacted broker URL metadata.
  - Evidence: `/opt/homebrew/bin/crystal spec spec/perf/t_perf_recov_001_spec.cr spec/perf/t_perf_gc_001_spec.cr spec/perf/t_perf_mem_001_002_spec.cr spec/perf/t_perf_stats_001_spec.cr spec/perf/t_perf_handshake_001_spec.cr spec/perf/t_perf_pub_001_spec.cr spec/perf/t_perf_pub_002_spec.cr spec/perf/t_perf_multichan_001_spec.cr spec/perf/t_perf_cons_001_spec.cr spec/docs_falsifier_link_spec.cr --error-trace` exits 0: 15 examples, 0 failures, 0 errors, 11 pending.
  - Evidence: `AMQP_PERF_LIVE=1 AMQP_PERF_RECOV_SAMPLES=1 AMQP_PERF_RECOV_001_MAX_MS=999999 /opt/homebrew/bin/crystal spec spec/perf/t_perf_recov_001_spec.cr --error-trace` exits 0 and remains pending when `AMQP_CHAOS_DOCKER_CONTAINER` is not supplied.
  - Evidence: `/opt/homebrew/bin/crystal build spec/perf/t_perf_recov_001_spec.cr --release --no-codegen --error-trace`, `/opt/homebrew/bin/crystal tool format --check src spec tools/perf_publish.cr tools/capture_proxy.cr tools/perf_smoke_assert.cr tools/perf_thresholds.cr tools/perf_threshold_assert.cr tools/perf_recommendations.cr tools/perf_recommend.cr tools/perf_compare_report.cr tools/perf_compare.cr`, and full `/opt/homebrew/bin/crystal spec --error-trace` exit 0; full suite reports 264 examples, 0 failures, 0 errors, 103 pending.
  - Cutline: this measures Docker broker-restart recovery only when a container is supplied; forced socket recovery, broker-specific restart variance, and CI-normalized baselines remain future work.
- [x] Apply the twenty-fifth higher-level LTP/WBA async-bytes confirm corridor move.
  - Frame: `Window` = public `publish_async(body : Bytes, ...)` on confirm-mode channels; `Transport` = caller bytes -> pending confirm registration -> publish frames -> async outcome channel; `Potential` = `(message_wrapper_allocations, replay_payload_correctness, async_confirm_latency)`.
  - Progress: bytes async confirms now write directly from `Bytes` and register a nil replay payload under `Recovery::None`; `Recovery::Full` still builds a replay `Message` before the write so reconnect replay semantics are preserved.
